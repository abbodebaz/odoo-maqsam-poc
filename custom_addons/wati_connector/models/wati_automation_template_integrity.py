...
class WatiAutomationRuleTemplateIntegrity(models.Model):
    ...
    def _hard_rebuild_template_parameters(self, reason="runtime"):
        ...
        for rule in self.sudo():
            ...
            _logger.warning(
                "WATI_TEMPLATE_INTEGRITY rule=%s active=%s template=%r reason=%s body_tokens=%s before=%s after=%s",
                rule.id,
                rule.active,
                rule.template_name,
                reason,
    ...
    def _repair_template_integrity_final(self):
        # Include draft/inactive automations. The builder intentionally creates
        # rules inactive until review, so excluding them leaves exactly the
        # records that most need repair untouched.
