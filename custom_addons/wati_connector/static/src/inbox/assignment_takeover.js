(() => {
    ...
    let busy = false;
    function selectedId() { return Number(localStorage.getItem("watiInboxSelected") || 0); }
    ...
    function makeActionButton(text, onClick, variant = "primary", disabled = false) {
        ...
        button.className = variant === "takeover" ? "wati-actions-secondary is-takeover" :
    ...
    async function assignMe(previousUserName = "") {
        const id = selectedId();
        if (!id || busy) return;
        if (previousUserName && !window.confirm(
            `The conversation is currently at ${previousUserName}.\n\nDo you want to receive it?`
        )) return;
        busy = true;
        ...
        try {
            const body = new URLSearchParams({ csrf_token: csrfToken, conversation_id: String(id) });
    ...
    function renderAssignment(data) {
        ...
        if (!data.wati_email) {
            ...
            statusValue.textContent = "WATI email not set";
            assignedUserValue.textContent = data.current_user_name || "—";
            actionBox.appendChild(makeActionButton("Add WATI Operator Email", null, "secondary", true));
            setComposerEnabled(false, "Add WATI Operator Email in your user account...");
            return;
        }
        ...
        if (data.is_unassigned) {
            ...
            assignedUserValue.textContent = "—";
            actionBox.appendChild(makeActionButton("Receive the conversation", () => assignMe()));
        ...
        if (data.can_takeover) actionBox.appendChild(makeActionButton(
            "Receive the conversation", () => assignMe(data.assigned_user_name), "takeover"
        ));
    ...
    async function refreshAssignment(forceRender = false) {
        const id = selectedId();
        if (!id) { renderEmptyState(); lastConversationId = 0; return; }
        try {
            ...
            const signature = JSON.stringify([
                data.assigned_user_id, data.assigned_to_me, data.wati_email,
                data.can_takeover, data.assigned_user_name, data.current_user_name,
            ]);
        ...
        } catch (error) { console.error("WATI assignment state error", error); }
    }
