...
def migrate(cr, version):
    """Normalize historical automation state when upgrading to 19.0.9.0.0.

    Migrations are intentionally versioned and idempotent. Product data files
    must never execute one-off repair functions during every module upgrade.
    """
    ...
    if _table_exists(cr, log_table) and all(_column_exists(cr, log_table, col) for col in required):
        cr.execute(
            """
